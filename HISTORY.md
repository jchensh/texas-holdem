...
## Step 9.2 — 牌型图例卡牌化展开、摊牌赢家明牌物理呈现、结算时间延长与高 DPI 字体放大（2026-05-23，commit `d38101a`）

**目标**：提升游戏对局终盘透明度与直观可读性，打造极致奢华且直观的德州终局体验，支持图例卡牌微缩化自适应展开，并优化大屏/高 DPI 环境下字号可读性。

**产出**：
- `public/index.html` —
  - 底部德州牌型规则添加了 `🔍 牌例：展开/收回` 独立切换按钮。
  - 新增 `#rules-card-showcase` 微缩扑克牌大厅（包含皇家同花顺至高牌全部 10 种手牌组合微缩物理卡牌组件，默认收起）。
  - 在大结算框内注入 `#settlement-winner-cards` 赢家物理扑克牌明牌展示槽。
- `public/style.css` —
  - 编写了超微型扑克牌样式 `.card.xs`，设计微缩花色和自适应字体，完美融入底栏悬浮面板。
  - 全面重构了大结算 `.settlement-overlay` 的版面，将其从顶部条状提示框升级为霸气震撼的 **中央磨砂玻璃拟态模态大结算窗**，极具视觉冲击力。
  - 全面升级全局字号：坐席玩家昵称（`.seat-name`）、筹码量（`.seat-chips`）、顶部状态行和底部操作面板整体字号加大 15%-25%；特别针对 Hero 本人底部的名字与筹码字号做发光和特大化处理，防止高 DPI/移动端眯眼看牌。
- `public/js/app.js` —
  - 绑定 `#btn-toggle-rules` 点击事件，实现手牌图例的平滑伸缩（默认隐藏，点击平滑展开）。
  - 精修结算流程 `_showSettlementOverlay(results)`：自动解析 `results.hands`。在最终进行 `showdown`（摊牌）时，提取赢家的实际 5 张赢牌底牌与公牌组合，在中央结算模态框中物理绘制渲染为带花色、发光描边的扑克牌实体图片，让人一眼看懂输赢原因。
  - 前端结算倒计时动画与移除定时器时间从原 `6.0s` 完美拉伸至 `7.5s` - `7.7s`，完美契合 8 秒节奏。
- `server/table.js` —
  - 将局终 showdown 结算的休眠清场延迟从 `6000` ms 调大至 `8000` ms，给玩家充分留足 8 秒钟看清牌型与数钱的快感。

**关键决策**：
- **微缩牌型大厅**：每个牌型不仅是文字，而是采用超微型扑克 `.card.xs`（25px宽，38px高）以物理图形形式展现。利用绝对定位悬浮在规则条上方，展开时不占用界面任何排版空间，默认收回，体验干净纯粹。
- **赢家物理明牌摊牌**：如果胜负由 Showdown 产生，结算时不仅文字宣告“同花赢了三条”，还会以实体精美扑克牌渲染出赢家的这 5 张组合牌（含高亮金色呼吸霓虹框），彻底免除“不知道别人赢的是什么牌”的痛点。
- **全局大字号适配**：针对玩家痛点，通过重写 CSS 基础字号，将所有关键数据放大，特别是 Hero 本人的信息，使其在高 DPI 屏下具备极其出色的远视辨识度。
...
| 9.2 | 牌型规则卡牌化展开、结算赢家底牌物化、倒计时延长至8s与高DPI字号放大 | ✅ commit `d38101a` |

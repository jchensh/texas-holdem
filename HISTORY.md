...
## Step 15 — 登录界面华丽化与轻量 3D 牌桌舞台（2026-05-26，待提交）

**目标**：在不引入构建工具、不改认证流程的前提下，把登录/注册页从静态背景升级为扑克主题沉浸式首屏，增强“Poker Night”开场气质。

**产出**：
- `public/index.html` — 在 `#view-auth` 内新增 `auth-fx` 装饰层，包含椭圆牌桌、牌桌灯光、扑克牌扇、旋转筹码与光粒子；全部 `aria-hidden`，不干扰表单语义和交互。
- `public/style.css` — 重写认证视图视觉系统：深色舞台背景、玻璃质感登录卡、金色高光按钮、发光品牌标识、CSS 3D 牌桌透视、筹码/卡牌/粒子动画、移动端适配与 `prefers-reduced-motion` 降级。
- `public/js/app.js` — 新增 `_initAuthEffects()`，用鼠标/指针位置只更新 CSS 变量，实现轻量视差；不触碰登录状态、socket 或认证 API。

**关键决策**：
- 不引入 Three.js / GLB 资产，先用原生 HTML/CSS/JS 做“轻量 3D”效果，符合当前前端无构建、无框架的项目边界。
- 登录和注册 tab 共用同一舞台；注册表单较高时用 `:has(#form-register.active)` 压缩间距和头像网格高度，避免 1280×720 与移动端视口贴边裁切。
- 动效层完全不可交互，真实 UI 仍是原生表单；低动效偏好下关闭动画和鼠标追踪。

**测试验证**：
- `node --check public/js/app.js` 通过。
- `npm.cmd test`：依赖安装后 68 个单元测试通过；3 个旧 E2E 失败是因为 `npm test` 直接运行时未预先启动 3010 服务/测试库目录（本次未改 E2E 依赖关系）。
- 内置浏览器验证 `http://127.0.0.1:3001/`：页面标题正确、`#view-auth.active` 渲染、控制台无 error/warn；登录 tab 输入框可填；注册 tab 可切换。
- 视觉验证：桌面 1280×720 登录/注册无横向溢出；注册卡片底部 686px、按钮底部 664px，均在 720px 视口内；移动 390×844 登录/注册无横向溢出，注册按钮底部 633px，表单完整可见。注册态压缩用 JS 切换 `.register-active`，避免依赖 CSS `:has()`。
